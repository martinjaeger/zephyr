...
/* maximum number of parallel frag sessions according to the standard */
#define FRAG_TRANSPORT_MAX_SESSIONS (4)
...
{
	uint8_t tx_buf[3 * FRAG_TRANSPORT_MAX_ANS_LEN];
	uint8_t tx_pos = 0;
	uint8_t rx_pos = 0;
	int ans_delay = 0;
...
#if CONFIG_LORAWAN_FRAG_TRANSPORT_VERSION == 1
...
#elif CONFIG_LORAWAN_FRAG_TRANSPORT_VERSION == 2
				tx_buf[tx_pos++] = FRAG_TRANSPORT_CMD_FRAG_STATUS;
				tx_buf[tx_pos++] = memory_error & 0x01;
				tx_buf[tx_pos++] = ctx.nb_frag_received & 0xFF;
				tx_buf[tx_pos++] =
