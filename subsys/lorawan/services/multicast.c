				       ...
				       uint8_t len, const uint8_t *rx_buf)
{
	...
	while (rx_pos < len) {
		...
		if (sizeof(tx_buf) - tx_pos < MAX_MULTICAST_ANS_LEN) {
			/*
			 * This has never happened during testing, but we still make sure to catch
			 * the error in case of different backend implementations.
			 */
	...
	if (tx_pos > 0) {
		...
		lorawan_services_schedule_uplink(LORAWAN_PORT_MULTICAST, tx_buf, tx_pos, delay);
	}
}
